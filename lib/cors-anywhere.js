...
function proxyRequest(req, res, proxy) {
  ...
  try {
    ...
    console.log(
      new Date().toISOString(), 'Proxy request:',req.corsAnywhereRequestState.location.href,
      '\n\tCPU load avg:', os.loadavg(),
      '\n\tMemory Used:', totalmem - os.freemem +'/'+ totalmem
    );
  } catch (err) {
    proxy.emit('error', err, req, res);
    console.warn(new Date().toISOString(),'Proxy error:', err);
  }
}
